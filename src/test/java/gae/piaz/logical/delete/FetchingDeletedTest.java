...
class FetchingDeletedTest {
    ...
    void testFetchDeletedAuthor(String endpoint) throws Exception {
        ...
        String response =
                mockMvc.perform(get("/api/authors" + endpoint + "/deleted/" + author.getId()))
                        .andExpect(status().isOk())
                        .andReturn()
                        .getResponse()
                        .getContentAsString();

        AuthorController.AuthorDTO dto =
                objectMapper.readValue(response, AuthorController.AuthorDTO.class);
